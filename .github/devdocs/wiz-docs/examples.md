...
> ⚠️ **패키지 버전 안전성 주의**: 예제의 패키지 사용 코드(ORM, Session, Service 등)는 **일반적인 패턴**을 보여줍니다. 세부 API는 패키지 버전에 따라 다를 수 있으므로, 구현 시 반드시 해당 패키지의 `src/portal/{package}/README.md`를 확인하십시오.
