...
> ⚠️ **패키지 버전 안전성 주의**: 이 문서들에 포함된 패키지 사용 예제(ORM, Session, Service 등)는 **일반적인 패턴과 구조**를 설명하는 참고 자료입니다. 패키지별 세부 API(메서드 시그니처, 파라미터, 반환값 등)는 버전마다 다를 수 있으므로, 구현 시 반드시 해당 패키지의 `src/portal/{package}/README.md`를 최종 권위 문서로 참조하십시오.
