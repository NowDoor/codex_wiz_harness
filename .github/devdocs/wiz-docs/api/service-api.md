...
> ⚠️ **패키지 버전 안전성 주의**: 이 문서의 Service API 예제는 **일반적인 구조와 패턴**을 설명합니다. 하위 모듈, 메서드, 파라미터 등 세부 API는 패키지 버전에 따라 다를 수 있으므로, 구현 시 반드시 해당 패키지의 `src/portal/{package}/README.md`를 확인하십시오.

## 클래스 정보

- **클래스**: `Service`
- **임포트**: `import { Service } from '@wiz/libs/portal/{package}/service';` (패키지명은 프로젝트에 맞게 대체)
