...
> ⚠️ **패키지 버전 안전성 주의**: 이 문서의 패키지 API 예제(ORM, Session, Service, season.util 등)는 **일반적인 호출 패턴**입니다. 세부 API는 패키지 버전에 따라 다를 수 있으므로, 구현 시 반드시 해당 패키지의 `src/portal/{package}/README.md`를 확인하십시오.
