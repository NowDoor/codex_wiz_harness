...
> ⚠️ **패키지 버전 안전성 주의**: 이 문서의 패키지 사용 예제(ORM, Session, Service, season.util 등)는 **일반적인 패턴과 구조**를 설명합니다. 패키지별 세부 API는 버전마다 다를 수 있으므로, 구현 시 반드시 해당 패키지의 `src/portal/{package}/README.md`를 최종 권위 문서로 참조하십시오.
